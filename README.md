...
#### Step 2. Create HyperTrack User & Start Mock Tracking Device Location
...
When the user is created, we need to `start mock tracking` his location and activity. Call the following method to do so ```HyperTrack.startMockTracking()```. Refer [here](https://docs.hypertrack.com/sdks/android/reference/hypertrack.html#void-startmocktracking) for more detail.
